...
# Orchestrator and Sub-agents : 사용자가 한 질문에 대해 하위 질문을 자동으로 생성하고, 하위질문들의 답변을 종합하여 최종적으로 사용자의 질문에 대한 답변을 제공
...
# 파이썬 f string에서는 {} 1 개는 변수, JSON에서는 2개를 사용해야함
...
async def orchestrate_task(user_query):
    ...
    orchestrator_response = llm_call(orchestrator_prompt, model="gpt-4o") # 사용자 질문을 기반으로 여러 질문을 도출

    # 응답 결과 (1단계 출력)
    print("\n============================orchestrator response==========================\n")
    print(orchestrator_response)

    response_json = json.loads(orchestrator_response.replace('```json', '').replace('```', '')) # 반환받은 답변을 json 형태로 변환
    
    analysis = response_json.get("analysis", "")
    sub_tasks = response_json.get("subtasks", [])

    # 2단계 : 각 하위 질문에 대한 LLM 호출
